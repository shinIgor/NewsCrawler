...
public class AccessTokenServiceImpl implements AccessTokenService {
    ...
    public TokenResponse createAccessToken(HttpServletRequest request, Account account) {
        ...
        List<SessionTokenDto> storedTokenDtoList = redisCacheService.getAllList(
            RedisCacheKeyDefine.getUsnToTokenKey(account.getId()), SessionTokenDto.class);
        for (SessionTokenDto dto : storedTokenDtoList) {
            redisCacheService.removeValue(
                RedisCacheKeyDefine.getAccessTokenKey(dto.getAccessToken().getValue()));
            redisCacheService.removeValue(
                RedisCacheKeyDefine.getRefreshTokenKey(dto.getRefreshToken().getValue()));
        }
        redisCacheService.removeValue(RedisCacheKeyDefine.getUsnToTokenKey(account.getId()));
        redisCacheService.setValue(RedisCacheKeyDefine.getAccessTokenKey(accessToken.getValue()),
            sessionTokenDto, 1, TimeUnit.HOURS);
        redisCacheService.setValue(RedisCacheKeyDefine.getRefreshTokenKey(refreshToken.getValue()),
            sessionTokenDto, 2, TimeUnit.HOURS);
        redisCacheService.leftPushList(RedisCacheKeyDefine.getUsnToTokenKey(account.getId()),
            sessionTokenDto, 3, TimeUnit.HOURS);
